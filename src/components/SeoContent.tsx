    ...
    >
      <div className="mx-auto max-w-3xl px-4 sm:px-6">
        <h1 className="text-2xl sm:text-3xl font-bold text-[var(--text-primary)] tracking-tight">
          {lang === 'en'
            ? 'SuperPixMia — Free Online Image Tools, 100% in Your Browser'
            : 'SuperPixMia — 免费在线图片工具，全程浏览器本地处理'}
        </h1>
        <p className="mt-4 text-sm sm:text-base leading-relaxed text-[var(--text-dim)]">
          {c.intro}
        </p>
        <p className="mt-3 text-sm sm:text-base leading-relaxed text-[var(--text-dim)]">
          <strong className="text-[var(--text-primary)] font-semibold">
            {lang === 'en' ? 'Privacy first: ' : '隐私优先：'}
          </strong>
          {c.privacy}
        </p>
        ...
        <div className="mt-10 grid gap-6 sm:grid-cols-2">
          {c.tools.map((tool) => (
            <article key={tool.title} className="glass rounded-xl border border-white/[0.06] p-5">
        ...
        <section className="mt-12">
          <h2 className="text-xl font-semibold text-[var(--text-primary)]">{c.howToTitle}</h2>
          <ol className="mt-4 space-y-4">
            {c.howToSteps.map((step, i) => (
              <li key={step.title} className="flex gap-3">
                <span className="mt-0.5 flex h-6 w-6 shrink-0 items-center justify-center rounded-full bg-[var(--accent)]/15 text-xs font-bold text-[var(--accent)]">
                  {i + 1}
                </span>
                <div>
        ...
        <section className="mt-12">
          <h2 className="text-xl font-semibold text-[var(--text-primary)]">{c.faqTitle}</h2>
          <div className="mt-4 space-y-5">
            {c.faqs.map((faq) => (
              <div key={faq.q}>
                <h3 className="text-sm font-semibold text-[var(--text-primary)]">{faq.q}</h3>
                <p className="mt-1 text-sm leading-relaxed text-[var(--text-dim)]">{faq.a}</p>
              </div>
            ))}
          </div>
        </section>
